        ...
        with TemporaryDirectory() as directory:
            ...
            with patch.object(image_tui, "DEFAULT_WORKFLOW_RUNS_ROOT", root):
                async with open_editor(graph, directory) as (app, editor, pilot, document_path):
                    ...
                    self.assertEqual(str(editor.query_one("#workflow-run", Button).label), "Generate choices")
                    ...
                    await editor.workers.wait_for_complete()
                    await pilot.pause()
                    self.assertEqual(str(editor.query_one("#workflow-run", Button).label), "Choose image")
                    button = editor.query_one("#workflow-run", Button)
                    self.assertGreaterEqual(button.content_size.width, button.label.cell_length)
                    self.assertTrue(await pilot.click("#workflow-run"))
                    await pilot.pause()
                    results = app.screen
                    ...
                    button = results.query_one("#candidate-1", ResultPreview)
                ...
                async with open_editor(saved, directory) as (app, editor, pilot, _):
                    ...
                    self.assertTrue(await pilot.click("#workflow-run"))
                    await pilot.pause()
                    self.assertIsInstance(app.screen, WorkflowResults)
                    self.assertEqual(app.screen.node_id, "next")
