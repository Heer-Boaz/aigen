...
def load_iro_corpus_config(path: Path) -> IroCorpusConfig:
    config_path = Path(path).expanduser().resolve()
    payload = read_json(config_path, label="iRO pix2pix corpus config")
    try:
        return IroCorpusConfig.model_validate(payload)
    except ValidationError as error:
        raise Pix2PixError(f"invalid iRO pix2pix corpus config: {error}") from error


def corpus_config_fingerprint(config: IroCorpusConfig) -> str:
    payload = json.dumps(
