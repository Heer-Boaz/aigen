...
- control.source is the extracted asset source and must be exactly "example_pose" or "example_contour".
...
- source "example_contour" must use type "canny" or "softedge".
- use residual_mask_source="example_boundary_mask" only for masked contour/silhouette control.
...
Choose control strengths from the image evidence. Strong pose control is useful when limb placement matters. Early, weaker contour control is useful when perspective and silhouette matter. Do not blindly copy fixed numeric examples.
